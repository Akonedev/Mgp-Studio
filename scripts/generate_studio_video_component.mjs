...
const TARGET_FILE = path.join(process.cwd(), 'packages/studio/src/components/MontageStudio.jsx');
...
export default function MontageStudio({ apiKey, onNavigateTab }) {
  ...
  const [videoClips, setVideoClips] = useState([
    {
      ...
      type: "video",
      track: "V1",
      url: "/assets/davinci/clip_truck_desert.png",
    ...
    },
    {
      ...
      type: "video",
      track: "V1",
      url: "/assets/davinci/clip_cowboy_dylan.png",
  ...
  const CAMERAS = [
    { id: "cam_1", label: "Camera A (Wide 24mm)", angle: "1", url: "/assets/davinci/clip_truck_desert.png" },
    { id: "cam_2", label: "Camera B (Close-Up 85mm)", angle: "2", tally: true, url: "/assets/davinci/clip_cowboy_dylan.png" },
  ...
  const filteredAssets = useMemo(() => {
    ...
    if (list.length === 0) {
      list = [
        { id: "b003_1", name: "B003_02101728_C001.braw", dur: "00:00:14:12", url: "/assets/davinci/clip_truck_desert.png", type: "video" },
        { id: "b003_2", name: "B003_02101729_C002.braw", dur: "00:00:28:06", url: "/assets/davinci/clip_cowboy_dylan.png", type: "video" },
    ...
    <div className="h-full flex flex-col bg-[#121214] text-zinc-200 select-none overflow-hidden font-sans border-t border-white/5 relative">
      ...
      <header className="flex-shrink-0 h-10 border-b border-white/[0.07] bg-[#18181b] px-3 flex items-center justify-between z-30">
        ...
        <div className="flex items-center gap-4">
          <div
            onClick={() => {
              const input = prompt("Entrer le timecode cible en secondes :", playheadTime.toFixed(2));
              if (input && !isNaN(Number(input))) setPlayheadTime(Number(input));
            }}
            ...
            title="Cliquer pour sauter à un timecode"
          >
                                ...
                                >
                                  <div className="aspect-video bg-black rounded overflow-hidden relative group">
                                    {item.type === "video" || item.filename?.endsWith(".mp4") ? (
                                      <img src={item.url?.replace(/\\.mp4$/, ".png") || "/assets/davinci/clip_truck_desert.png"} alt="" className="w-full h-full object-cover" />
                                      ...
                                      <img src={item.url || "/assets/davinci/clip_cowboy_dylan.png"} alt="" className="w-full h-full object-cover" />
                                    )}
                  ...
                  <div className="flex-1 bg-black flex items-center justify-center relative overflow-hidden">
                    ...
                    >
                      {activeVideoClip?.url?.endsWith(".mp4") ? (
                        ...
                        <img
                          src={activeVideoClip?.url || "/assets/davinci/clip_cowboy_dylan.png"}
                          alt="Timeline Monitor"
                          className="max-h-full max-w-full object-contain"
                        />
                      )}

                      {/* Active Title Overlay from Track V3 */}
                      {activeTitleClip && (
              ...
              {showInspector && (
                <div className="w-80 flex-shrink-0 flex flex-col border-l border-white/[0.07] bg-[#141416] overflow-y-auto custom-scrollbar">
                  ...
                  <div className="h-7 border-b border-white/[0.07] px-2 flex items-center justify-between text-[10px] bg-[#111113]">
                    {["video", "audio", "effects", "transitions", "file"].map(t => (
                      <button
                        key={t}
                  ...
                  >
                    ...
                    <div className="h-10 relative bg-yellow-950/10 p-1">
                      {titleClips.map(clip => (
                        <div
                          ...
                          className={\`absolute top-1 h-7 rounded px-2 flex items-center justify-between text-[9px] font-bold cursor-pointer transition-all \${
                        ...
                        >
                          ...
                          <span className="text-[7px] text-yellow-400 ml-1">{clip.duration}s</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="h-10 relative bg-blue-950/10 p-1">
                      {videoClips.filter(c => c.track === "V2").map(clip => (
                        <div
                          key={clip.id}
                          onClick={(e) => { e.stopPropagation(); handleSelectClip(clip); }}
                          className={\`absolute top-1 h-7 rounded px-2 flex items-center justify-between text-[9px] cursor-pointer transition-all \${
                            selectedClip?.id === clip.id
                        ...
                        >
                          <span className="truncate">{clip.title || clip.name}</span>
                          <span className="text-[7px] text-blue-300 ml-1">{clip.duration}s</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="h-12 relative bg-orange-950/10 p-1">
                      {videoClips.filter(c => c.track === "V1" || !c.track).map(clip => (
                        <div
                          ...
                          className={\`absolute top-1 h-9 rounded flex items-center overflow-hidden p-0.5 gap-1.5 cursor-pointer transition-all \${
                            selectedClip?.id === clip.id
                        ...
                        >
                          <img src={clip.url || "/assets/davinci/clip_truck_desert.png"} alt="" className="h-full aspect-video object-cover rounded" />
                    ...
                    <div className="h-10 relative bg-amber-950/20 p-1">
                      {voiceClips.map(clip => (
                        <div
                          key={clip.id}
                          onClick={(e) => { e.stopPropagation(); handleSelectClip(clip); }}
                          className={\`absolute top-1 h-7 rounded px-2 flex items-center justify-between cursor-pointer transition-all \${
                            selectedClip?.id === clip.id
                        ...
                        >
                          <span className="text-[9px] font-mono text-amber-200 font-bold truncate mr-2">{clip.name}</span>
                    ...
                    <div className="h-10 relative bg-emerald-950/20 p-1">
                      {soundtrackClips.filter(c => c.track === "A2" || !c.track).map(clip => (
                        ...
                        >
                          <span className="text-[9px] font-mono text-emerald-200 font-bold truncate mr-2">{clip.name}</span>
                          <div className="flex items-center gap-0.5 h-4 flex-shrink-0">
                    ...
                    <div className="h-10 relative bg-cyan-950/20 p-1">
                      {soundtrackClips.filter(c => c.track === "A3").map(clip => (
                        <div
                          key={clip.id}
                          onClick={(e) => { e.stopPropagation(); handleSelectClip(clip); }}
                          className="absolute top-1 h-7 rounded bg-cyan-600/30 border border-cyan-500/60 px-2 flex items-center justify-between cursor-pointer"
                          style={{
                            left: \`\${clip.start * timelineZoom}px\`,
                            width: \`\${clip.duration * timelineZoom}px\`,
                          }}
                        >
                          <span className="text-[9px] font-mono text-cyan-200 font-bold truncate">{clip.name}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="h-10 relative bg-yellow-950/20 p-1">
                      {sfxClips.map(clip => (
                        ...
                        >
                          <span className="text-[9px] font-mono text-yellow-300 font-bold truncate mr-1">{clip.name}</span>
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-[#0c0c0e]">
            ...
            <div className="flex-1 flex min-h-0">
              <div className="w-72 border-r border-white/[0.07] bg-[#141416] flex flex-col p-2 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-1 overflow-y-auto flex-1 custom-scrollbar">
                  {[1, 2, 3, 4, 5, 6, 7, 8].map(n => (
                    ...
                    >
                      <img src="/assets/davinci/photo_model_red_dress.png" alt="" className="w-full h-full object-cover" />
              ...
              <div className="flex-1 bg-black flex items-center justify-center p-4 relative overflow-hidden">
                <img
                  src="/assets/davinci/photo_model_red_dress.png"
                  alt="Fashion Shoot"
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-[#09090b]">
            ...
            <div className="h-[46%] flex border-b border-white/[0.07]">
              <div className="flex-1 bg-black flex items-center justify-center p-2 relative overflow-hidden">
                <img
                  src="/assets/davinci/fusion_documentary_spiral.png"
        ...
        {activePage === "color" && (
          <div className="flex-1 flex flex-col min-h-0 bg-[#09090b]">
            <div className="h-[46%] flex border-b border-white/[0.07]">
                ...
                <div className="grid grid-cols-3 gap-1 overflow-y-auto flex-1 custom-scrollbar">
                  {["1.64.3", "1.64.4", "1.64.5", "1.88.1", "1.95.1", "1.105.1"].map(id => (
                    <div key={id} className="aspect-video bg-black rounded border border-white/10 overflow-hidden relative cursor-pointer hover:border-amber-400">
                      <img src="/assets/davinci/clip_cowboy_dylan.png" alt="" className="w-full h-full object-cover" />
              ...
              <div className="flex-1 bg-black flex items-center justify-center p-2 relative overflow-hidden">
                <img
                  src="/assets/davinci/color_grade_monitor.png"
        ...
        {activePage === "fairlight" && (
          <div className="flex-1 flex flex-col min-h-0 bg-[#09090b] relative">
            <div className="h-28 border-b border-white/[0.07] bg-[#111113] p-1.5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 h-full pl-2 border-l border-white/10">
                ...
                <div className="w-36 h-full bg-black rounded border border-white/10 overflow-hidden relative">
                  <img src="/assets/davinci/fairlight_band_monitor.png" alt="Live Band" className="w-full h-full object-cover" />
            ...
            <div className="flex-1 flex min-h-0 bg-[#0c0c0e]">
              ...
              {/* Console Mixer */}
              <div className="w-64 border-l border-white/[0.07] bg-[#141416] p-2 flex flex-col justify-between font-mono text-[9px]">
                <span className="font-bold text-zinc-300 block mb-1 font-sans">Fairlight Mixer</span>
                <div className="flex gap-2 flex-1 my-1">
                  {["A1", "A2", "A4", "Bus 1"].map(ch => (
                    <div key={ch} className="flex-1 flex flex-col items-center justify-between">
                      <span className="font-bold text-zinc-300">{ch}</span>
                      <div className="w-3 flex-1 bg-black/60 rounded flex flex-col justify-end p-0.5 border border-white/5 my-1">
                        <div className="w-full bg-emerald-400" style={{ height: "75%" }} />
                      </div>
                      <span className="text-[8px] text-zinc-400">-6dB</span>
                    </div>
                  ))}
                </div>
              </div>
...
console.log('Successfully generated updated MontageStudio.jsx at:', TARGET_FILE);
